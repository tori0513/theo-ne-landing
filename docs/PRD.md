...
두 카드가 16px 아래에서 제자리로 올라오며 나타나고(0.7s, 오른쪽 카드 0.08s 지연), 이어서 연결 행이
페이드인한다. 2026-09-11 개편으로 원근 회전·SVG 선 그리기·태그 이동은 **제거**했다 — Meta 문법은 평면이며,
움직임은 상태 전환 150–250ms 수준에 그친다.

구현 제약:
- 실행 상태(`go`)는 **React state**로 관리한다. DOM에 클래스를 직접 붙이면 재렌더가 지운다
- 시작 상태(`transform`/`opacity`)는 `<html class="js">` 아래에서만 적용한다. JS를 실행하지 않는
  클라이언트는 완성 상태를 본다
- 860px 이하에서는 연결 행의 선만 숨기고 칩과 라벨은 남긴다
